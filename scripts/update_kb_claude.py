...
from run_claude_step import run_claude_step
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Update the markdown KB incrementally using Claude Code for LLM steps."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument("--claude-bin", default="claude", help="Claude Code CLI binary to invoke.")
    parser.add_argument("--model", help="Optional Claude model override (e.g. claude-opus-4-7).")
    parser.add_argument(
        "--permission-mode",
        default="acceptEdits",
        choices=["default", "plan", "acceptEdits", "bypassPermissions"],
        help="Permission mode when not using --dangerously-skip-permissions.",
    )
    parser.add_argument(
        "--no-skip-permissions",
...
def run_claude_layer(
    ...
    *,
    ...
    if not result.success:
        detail = result.stderr.strip() or result.stdout.strip() or f"`claude -p` returned {result.returncode}"
        step_log.append(
            ExecutedStep(
                name=layer,
                success=False,
...
def print_execution_plan(dirty_layers: list[str]) -> None:
    ...
    for layer in dirty_layers:
        mode = "claude -p" if layer in LLM_LAYERS else "deterministic script"
...
def main() -> int:
    ...
    for index, layer in enumerate(active_dirty_layers):
        ...
        if layer == "extraction":
            ...
            ok, working_snapshot, changes_by_layer = run_claude_layer(
                layer,
                report=report,
