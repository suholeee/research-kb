...
from run_claude_step import build_command, extract_agent_message
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Suggest citations for a scientific paragraph using local KB retrieval plus Claude Code."
    )
    parser.add_argument(
        "input_path",
        nargs="?",
    ...
    parser.add_argument("--dry-run", action="store_true", help="Render retrieval and prompt, but skip Claude Code invocation.")
...
def run_claude_prompt(
    ...
    *,
    ...
    if shutil.which(claude_bin) is None:
        raise RuntimeError(f"Could not find `{claude_bin}` on PATH.")
    ...
    return agent_message, completed.stdout, completed.stderr, completed.returncode, display_command
...
def main() -> int:
    ...
    try:
        agent_message, stdout, stderr, returncode, command = run_claude_prompt(
            prompt_text,
    ...
    if returncode != 0:
        print("Claude Code invocation failed.", file=sys.stderr)
